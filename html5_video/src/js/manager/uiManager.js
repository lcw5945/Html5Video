 ...
 */
flex.module('manager.uiManager').define(() => {
    var config = null,
        player = null,
        fullscreen = null;

    flex.Class.UIManager = class {
        constructor() {
        ...
        showCover() {
            let playerCover = flex.newElement('div');
    ...
    function _updatePopDisplay() {
        let element = player.popLayer.firstChild,
            w, h, left, top;
        if (player.isFullscreen) {
            w = window.innerWidth;
            h = window.innerHeight;
    ...
    function _updateControlsDisplay() {
        ...
        let tipEle = flex.getElement(config.selectors.buttons.fullscreen + ' .' + config.classes.tooltip);
        if (player.isFullscreen) {
            tipEle.innerText = "退出";
        } else {
            tipEle.innerText = "全屏";
    ...
    function _initVideoQualityList() {
        let videoData = flex.data["videoData"];
        let dicQ = {
            1: "标清",
            2: "高清",
            3: "超清"
        ...
        if (videoData.length > 0) {
            let ele = flex.getElement('.q-label');
            ele.innerText = dicQ[videoData.currentVD.quality];
            let ulEle = flex.newElement('ul');
            flex.each(videoData.splits, function (i, vd) {
                let liEle = flex.newElement('li');
    ...
    function _initPopLayer() {
        let popLayer = flex.newElement('div');
    ...
    function _initControlUI() {
        ...
        if (config.tooltips) {
            let labels = flex.getElements(config.selectors.labels);
