 ...
 */
flex.module('core.ajax').requires('core.promise').define(function(){
    ...
    var ajax = flex.ajax = function(options){
        var settings = flex.merge({}, options || {});
        flex.each(ajax.settings, function(i, value){
        ...
        if (!settings.crossDomain) {
            settings.crossDomain = /^([\w-]+:)?\/\/([^\/]+)/.test(settings.url) &&
                RegExp.$2 != flex.global.location.host;
        }
        ...
        //设置请求头
        settings.headers = flex.merge(baseHeaders, settings.headers || {});
        ...
        xhr.onreadystatechange = function(){
            if (xhr.readyState == 4) {
                ...
                if ((xhr.status >= 200 && xhr.status < 300) || xhr.status == 304 || (xhr.status == 0 && protocol == 'file:')) {
                    ...
                    try {
                        if (dataType == 'script') { eval(result); }//   (1,eval)(result); //context global
        ...
        if ( support.cors || xhr && !settings.crossDomain) {
            ...
            //}
            flex.each(settings.headers, function(name, value){
    ...
    ajax.JSONP = function(options){
        ...
        //终止函数
            abort = function(){
                ...
                flex.remove(script);
                //清空回调函数
                if (callbackName in window) { window[callbackName] = _empty; }
                //ajax请求完成
                ajaxComplete('abort', xhr, options)
            },
            xhr = { abort: abort }, abortTimeout,
            head = flex.getElement('head')
                || document.documentElement; //document.getElementsByTagName("head")[0]
    ...
    function triggerGlobal(settings, eventName, data) {
        if (settings.global){
            //返回响应函数处理结果
            return flex.fire(eventName, data);
        }
    }
