...
flex.module('core.ajax').requires('core.promise').define(function () {
    ...
    var ajax = flex.ajax = function (options) {
        ...
        if (!settings.crossDomain) {
            settings.crossDomain = /^([\w-]+:)?\/\/([^\/]+)/.test(settings.url) && RegExp.$2 != flex.global.location.host;
        }
    ...
    ajax.JSONP = function (options) {
        ...
        abort = function abort() {
            ...
            xhr = { abort: abort },
            abortTimeout,
            head = flex.getElement('head') || document.documentElement; //document.getElementsByTagName("head")[0]
